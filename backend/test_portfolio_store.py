...
def add_filled_order(connection: sqlite3.Connection, *, price: float, order_id: str) -> dict:
    preview = {
...
def main() -> None:
    ...
    assert snapshot["total_cost_basis"] == 621.86
    assert snapshot["market_value"] == 700.0
    assert snapshot["total_exposure"] == 700.0
    assert snapshot["unrealized_pnl"] == 78.14
    ...
    assert snapshot["positions"][0]["market_value"] == 700.0
    assert snapshot["positions"][0]["unrealized_pnl"] == 78.14
