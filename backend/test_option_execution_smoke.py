...
def main() -> None:
    ...
    try:
        ...
        preview = client.post(
            "/paper/preview",
            json=option_preview_request(strategy="COVERED_CALL", option_type="CALL", strike=210.0, price=3.50),
        )
        ...
        approval = client.post("/paper/approval", json={"preview": preview_payload, "approved": True})
        ...
        execution = client.post(f"/paper/execute/{queue_id}", json={"confirmation_phrase": "EXECUTE PAPER"})
        ...
        unsupported_preview = client.post(
            "/paper/preview",
            json=option_preview_request(strategy="IRON_CONDOR", option_type="CALL", strike=210.0, price=1.0),
        )
        ...
        assert unsupported_approval_payload["approval"]["option_structure"]["reason"] == "unknown_structure"
        ...
        margin_approval = client.post("/paper/approval", json={"preview": margin_preview_payload, "approved": True})
        ...
        put_preview = client.post(
            "/paper/preview",
            json=option_preview_request(strategy="CASH_SECURED_PUT", option_type="PUT", strike=190.0, price=2.10),
        )
        put_preview_payload = put_preview.json()["preview"]
        put_approval = client.post("/paper/approval", json={"preview": put_preview_payload, "approved": True})
        ...
        assert put_approval_payload["approval"]["option_structure"]["reason"] == "insufficient_cash_collateral"
    ...
    print("PASS: option order chain approves, blocks, and executes correctly end-to-end via the real API.")
