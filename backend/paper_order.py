...
def prepare_paper_order(*, symbol: str, quantity: int, price: float, position_pct: float, total_exposure_pct: float, loss_per_trade_pct: float, daily_loss_pct: float, orders_today: int) -> dict:
    ...
    risk = check_order(position_pct=position_pct, total_exposure_pct=total_exposure_pct, loss_per_trade_pct=loss_per_trade_pct, daily_loss_pct=daily_loss_pct, orders_today=orders_today)
    if risk["status"] != "PASS":
